...
const prisma =
  ...
  (() => {
    ...
    if (process.env.DATABASE_CONNECTION_LIMIT) {
      prismaOptions.__internal = {
        ...
        engine: {
          ...
          pool_timeout: parseInt(
            process.env.DATABASE_POOL_TIMEOUT || "20000",
            10
          ),
          socket_timeout: parseInt(
            process.env.DATABASE_SOCKET_TIMEOUT || "30000",
            10
          ),
        },
      };
    ...
    return new PrismaClient(prismaOptions as any);
  })();
    ...
    ? (() => {
        ...
        if (process.env.DATABASE_CONNECTION_LIMIT) {
          replicaOptions.__internal = {
            useTransactionApi: false,
            engine: {
              connection_limit: Math.floor(
                parseInt(process.env.DATABASE_CONNECTION_LIMIT, 10) * 0.5
              ), // 50% of primary pool
              pool_timeout: parseInt(
                process.env.DATABASE_POOL_TIMEOUT || "20000",
                10
        ...
        return new PrismaClient(replicaOptions as any);
      })()
...
export const checkDatabaseConnection = async (): Promise<{
  ...
  const results = {
    primary: false,
    replica: false,
    latency: { primary: 0, replica: undefined as number | undefined },
  };
...
export const getPgBouncerStats = async (): Promise<any> => {
  try {
