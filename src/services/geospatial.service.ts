...
export class GeospatialService {
   ...
   * Find nearby communities
   */
  private static async findNearbyCommunities(query: NearbyQuery): Promise<any[]> {
    ...
    const communitiesWithDistance = communities
      .map(community => {
        if (!community.owner.profile?.latitude || !community.owner.profile?.longitude) return null;
   ...
   */
  private static calculateBoundingBox(lat: number, lng: number, radiusKm: number) {
   ...
   */
  static async getUsersNearUser(userId: string, radiusKm: number = 50): Promise<any[]> {
   ...
   */
  static async areUsersInRange(userId1: string, userId2: string): Promise<boolean> {
    try {
   ...
   */
  static async getDiscoveryFeed(userId: string, limit: number = 20): Promise<any[]> {
    try {
