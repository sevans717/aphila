...
router.get('/version',
  async (req, res) => {
    try {
      ...
      const latest = platform ? versionInfo.latest[platform as keyof typeof versionInfo.latest] : null;
      const minimum = platform ? versionInfo.minimum[platform as keyof typeof versionInfo.minimum] : null;
  ...
  async (req, res) => {
    try {
      const status = {
        server: 'online',
        database: 'connected',
        storage: env.s3BucketName ? 's3' : 'local',
        features: {
