...
router.post('/register-device',
  ...
  async (req, res) => {
    try {
      ...
      const pushService = (global as any).pushNotificationService as PushNotificationService;
...
router.get('/',
  ...
  async (req, res) => {
    try {
      ...
      const result = await NotificationService.list(userId, { cursor, limit: limit ? parseInt(limit, 10) : undefined });
