...
router.get("/collections", auth, async (req, res) => {
  try {
    const collections = await BookmarkService.getUserCollections(
      req.user!.userId
    );
...
router.post(
  ...
  validateRequest({
    ...
    body: z.object({ collectionId: z.string().optional() }),
  }),
  async (req, res) => {
    try {
...
router.post(
  "/media/:mediaId/toggle",
  auth,
  validateRequest({ params: z.object({ mediaId: z.string().min(1) }) }),
  async (req, res) => {
    try {
