...
router.get("/feed", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const userId = req.user.userId;
        const result = await post_service_1.PostService.getFeed(userId, {
            cursor: cursor,
...
// Create a post
router.post("/", auth_1.auth, (0, validate_1.validateRequest)(createPostSchema), async (req, res) => {
    try {
        const userId = req.user.userId;
...
// Update a post
router.patch("/:postId", auth_1.auth, (0, validate_1.validateRequest)(updatePostSchema), async (req, res) => {
    try {
        const userId = req.user.userId;
        const updated = await post_service_1.PostService.updatePost(req.params.postId, userId, {
            content: req.body.content,
