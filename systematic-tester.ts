...
class SystematicTester {
  ...
  private logResult(result: TestResult): void {
    ...
    console.log(
      `${emoji} [${result.category}] ${result.testName} (${result.duration}ms)`
    );
    if (result.error) {
  ...
  private async testDatabaseConnection(): Promise<TestResult> {
    try {
      // Test database via Prisma singleton if available
      let prisma: any;
      try {
        prisma = require("./src/lib/prisma").prisma;
      } catch (e) {
        // fallback to creating a client directly
  ...
  private async testGetUnreadCount(): Promise<TestResult> {
    try {
      ...
      const response = await axios.get(
        `${this.apiUrl}/messaging/unread-count`,
        {
          headers: { Authorization: `Bearer ${this.testData.token}` },
          timeout: 10000,
        }
  ...
  private async testGetPlans(): Promise<TestResult> {
    try {
      const response = await axios.get(`${this.apiUrl}/subscription/plans`, {
        timeout: 10000,
      });
      const payload: any = (response as any).data;
      const plansArray = Array.isArray(payload)
        ? payload
      ...
      return {
        ...
        error: !Array.isArray(plansArray)
          ? "Plans should be an array (either response body or data.data)"
          : undefined,
        duration: 0,
      };
    } catch (error: any) {
  ...
  private async testGetCommunities(): Promise<TestResult> {
    try {
      ...
      let response: any;
      try {
        response = await axios.get(`${this.apiUrl}/communities`, {
          timeout: 10000,
        });
      } catch (e) {
        response = await axios.get(`${this.apiUrl}/discovery/communities`, {
          timeout: 10000,
        });
      }
      const data: any = (response as any).data;
      return {
        ...
        status:
          Array.isArray(data) || Array.isArray(data?.data) ? "PASS" : "FAIL",
        error: !(Array.isArray(data) || Array.isArray(data?.data))
          ? "Communities should be an array"
          : undefined,
        duration: 0,
      };
    } catch (error: any) {
  ...
  private async testServiceFunctions(): Promise<TestResult[]> {
    ...
    for (const serviceFile of serviceFiles) {
      try {
        ...
        const functionMatches = serviceContent.match(
          /export\s+(async\s+)?function\s+(\w+)|(\w+)\s*:\s*(async\s+)?function|class\s+\w+\s*{[^}]*(\w+)\s*\(/g
        );
        ...
        if (functionMatches) {
          ...
          serviceTests.push({
            ...
            duration: 0,
            suggestions: [
              "Add exportable functions to service",
  ...
  private getTestSuites(): TestSuite[] {
    return [
      ...
      {
        name: "Messaging",
        tests: [() => this.testSendMessage(), () => this.testGetUnreadCount()],
      },
      {
        name: "Subscription",
        tests: [
      ...
      {
        name: "WebSocket",
        tests: [() => this.testWebSocketConnection()],
      },
    ];
  }
  ...
  public async runAllTests(maxIterations: number = 3): Promise<void> {
    ...
    console.log(
      `🎯 Starting Systematic Test Suite (max ${maxIterations} iterations)\n`
    );
  ...
  private generateReport(iteration: number): void {
    ...
    console.log(
      `📝 Detailed report saved: test-report-iteration-${iteration}.json`
    );
  }
  ...
  private async attemptAutoFixes(): Promise<void> {
    ...
    const uniqueSuggestions = [
      ...new Set(failedResults.flatMap((r) => r.suggestions || [])),
    ];
